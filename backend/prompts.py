...
- Template 07 (comparison): 3–5 rows of criteria. col-b is the "target" side (accent-highlighted); col-a is the contrast. For the `.caption-note` at the bottom (starts with "※ "):
  - If col-a refers to a **생소하거나 전문적인 용어** (jargon, technical term, proper noun, or concept the general Korean reader might not know), write a one-line definition: `※ {{col-a 이름}}: {{20자 내외 간단 정의}}`.
  - If col-a is a **common everyday term** already familiar to readers (e.g. 초보자, 아침, 사과), skip the definition and fall back to a one-line thematic insight contrasting the two sides.

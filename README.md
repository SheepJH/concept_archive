...
모르는 개념이 생길 때마다 텔레그램 봇에 *"더닝-크루거 효과"* 처럼 한 줄을 보내면, 약 90초 뒤 봇이 **나만의 과외 노트 같은 카드뉴스 8장**을 앨범으로 답장합니다. 이해가 됐다 싶으면 `📤 인스타 아카이브` 버튼을 눌러 [@what_is_this.zip](https://instagram.com/what_is_this.zip)에 남겨둡니다 — 나중에 다시 꺼내 볼 수 있게.
...
<p align="center">
  <b>텔레그램 입력 → AI 카드 설계 → HTML 렌더링 → 텔레그램 프리뷰 → 인스타 자동 발행</b>
</p>
...
"구조화된 입력 → LLM 가공 → 디자인된 시각 자료 → 채널 푸시"가 필요한 다양한 사내 업무에 그대로 응용 가능합니다.

- 매일 아침 리서치 키워드를 카드형 트렌드 리포트로 슬랙에 자동 발송
- 사내 위키/노션 페이지를 신규 입사자용 온보딩 카드뉴스로 변환
- 회의록 텍스트를 요약 슬라이드 5장으로 변환해 채널 공유
- 마케팅 캠페인 카피를 브랜드 톤 카드 디자인으로 자동 렌더링 후 검수
